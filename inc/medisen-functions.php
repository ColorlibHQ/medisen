...
=========================================================*/
function medisen_opt( $id = null, $default = '' ) {
...
function medisen_site_icon(){
	if ( ! has_site_icon() ) {
		$html = '';
		$icon_path = MEDISEN_DIR_ASSETS_URI . 'img/favicon.png';
...
=========================================================*/
function medisen_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_medisen_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function medisen_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'medisen_excerpt_length' ) ) {
	function medisen_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'medisen_posted_comments' ) ) {
    function medisen_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','medisen');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','medisen');
            } else {
                $comments = esc_html__( '1 Comment','medisen' );
            }
            $comments = '<i class="ti-comments"></i>'. $comments;
        } else {
            $comments = esc_html__( 'Comments are closed', 'medisen' );
        }
...
/*===================================================
	Post embedded media
===================================================*/
function medisen_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function medisen_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'medisen' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'medisen' ) . ' </span>%',
...
function medisen_social_icon_overwrite_by_flaticon( $social_icon ){
	switch ( $social_icon ) {
...
/*====================================================
	Theme logo
====================================================*/
function medisen_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'medisen_logo_169x31' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="medisen logo"></a>';
	}else{
...
/*================================================
    Page Title Bar
================================================*/
function medisen_page_titlebar() {
	if ( ! is_page_template( 'template-builder.php' ) ) {
        ...
        <section class="hero-banner d-flex align-items-center">
            <div class="container">
				...
				<?php
				if ( is_category() ) {
					single_cat_title( __('Category: ', 'medisen') );
					...
					echo esc_html__( 'Search for: ', 'medisen' ) . get_search_query();

				} elseif ( ! ( is_404() ) && ( is_single() ) || ( is_page() ) ) {
					echo  get_the_title();

				} elseif ( is_home() ) {
					echo esc_html__( 'Blog', 'medisen' );

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'medisen' );
                ...
                <nav aria-label="breadcrumb" class="banner-breadcrumb">
					<?php
					if ( function_exists( 'medisen_breadcrumbs' ) ) {
						medisen_breadcrumbs();
					}
					?>
                </nav>
...
=================================================*/
function medisen_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function medisen_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function medisen_featured_post_cat(){
...
======================================================*/
function medisen_post_cat_name_only(){
...
========================================================*/
function medisen_sidebar_opt(){

    $sidebarOpt = medisen_opt( 'medisen_blog_layout' );
    $sidebar = '1';
...
/**================================================
	Themify Icon
 =================================================*/
function medisen_themify_icon(){
    return[
        'cap'     => __('Icon Cap', 'medisen'),
        'bag'     => __('Icon Bag', 'medisen'),
        'shirt'   => __('Icon T-shirt', 'medisen'),
        'cafe'    => __('Icon Cafe', 'medisen'),
    ];
}
...
============================================================*/
function medisen_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'medisen_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function medisen_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'medisen' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'medisen' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('medisen_blog_single_post_navigation') ) {
	function medisen_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'medisen_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'medisen' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'medisen' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'medisen_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Medisen Comment Template Callback
 ====================================================*/
function medisen_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'medisen' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'medisen'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'medisen' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'medisen' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'medisen_replace_reply_link_class');
function medisen_replace_reply_link_class( $class ) {
...
===========================================================*/
function medisen_latest_blog( $pNumber = 3, $post_order = 'DESC' ){
		...
		<div class="col-sm-6 col-lg-4 col-xl-4">
				<div class="single-home-blog">
					<div class="card">
						<?php
							if( has_post_thumbnail() ){
								the_post_thumbnail( 'medisen_home_blog_360x371', ['class' => 'card-img-top', 'alt' => get_the_title() ] );
							}
						?>
						<div class="card-body">
							<ul>
								<li> <span class="ti-user"></span><?php echo get_the_author_meta( 'display_name' )?></li>
								<li> <span class="ti-bookmark"></span><?php echo medisen_post_cat_name_only(); ?></li>
							</ul>
							<a href="<?php the_permalink(); ?>">
								<h5 class="card-title"><?php the_title(); ?> </h5>
							</a>
							<a href="<?php the_permalink(); ?>" class="blog_btn"><?php echo esc_html__( ' read more', 'medisen' ); ?></a>
...
/*=========================================================
    Share Button Code
===========================================================*/
function medisen_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
