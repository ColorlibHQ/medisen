...
/**
 *
 * Medisen elementor Doctors section widget.
 *
 * @since 1.0
 */
class Medisen_Doctors extends Widget_Base {

	public function get_name() {
		return 'medisen-doctors';
	}

	public function get_title() {
		return __( 'Doctors', 'medisen' );
	}
	...
	protected function _register_controls() {
        ...
        $this->start_controls_section(
            'doc_heading',
            [
                'label' => __( 'Doctors Heading', 'medisen' ),
            ]
        );
        $this->add_control(
            'doc_header',
            [
                'label'         => esc_html__( 'Section Header', 'medisen' ),
                'description'   => esc_html__('Use <br> tag for line break', 'medisen'),
                'type'          => Controls_Manager::WYSIWYG,
                'label_block'   => true,
                'default'       => __( '<h2> Experienced Doctors</h2><p>Face replenish sea good winged bearing years air divide wasHave night male also</p>', 'medisen' )
            ]
        );
		...
		$this->start_controls_section(
			'docs_block',
			[
				'label' => __( 'Doctors', 'medisen' ),
			]
		);
		$this->add_control(
            'docs_content', [
                'label' => __( 'Create a Doctor', 'medisen' ),
                'type'  => Controls_Manager::REPEATER,
                'title_field' => '{{{ doc_name }}}',
                'fields' => [
                    [
                        'name'  => 'image',
                        'label' => __( 'Doctor Image', 'medisen' ),
                        'type'  => Controls_Manager::MEDIA,
                    ],
                    [
                        'name'  => 'doc_name',
                        'label' => __( 'Name', 'medisen' ),
                        'type'  => Controls_Manager::TEXT,
                        'label_block' => true,
                        'default' => __( 'DR Adam Billiard', 'medisen' )
                    ],
                    [
                        'name'      => 'desg',
                        'label'     => __( 'Designation', 'medisen' ),
                        'type'      => Controls_Manager::TEXT,
                        'default'   => __( 'Heart specialist', 'medisen' )
                    ],
                    [
                        'name'      => 'fb',
                        'label'     => __( 'Facebook URL', 'medisen' ),
                    ...
                    [
                        'name'      => 'ins',
                        'label'     => __( 'Instagram URL', 'medisen' ),
                        'type'      => Controls_Manager::URL,
                        'show_external' => false,
                        'default'   => [
                            'url'   => '#',
        ...
        $this->start_controls_section(
            'color_sect', [
                'label' => __( 'Style Doctors Heading', 'medisen' ),
        ...
        $this->start_controls_section(
            'single_item_style', [
                'label' => __( 'Single Item Style', 'medisen' ),
                'tab' => Controls_Manager::TAB_STYLE,
            ]
        );
        ...
        $this->add_control(
            'doc_name_col', [
                'label'     => __( 'Doctor Name Color', 'medisen' ),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
        ...
        $this->add_control(
            'doc_des_col', [
                'label'     => __( 'Doctor Designation Color', 'medisen' ),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
        ...
        $this->add_control(
            'hov_doc_name_col', [
                'label'     => __( 'On Hover Doctor Name Color', 'medisen' ),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .doctor_part .single_blog_item:hover h3' => 'color: {{VALUE}};',
                ],
            ]
        );   
        ...
        $this->add_control(
            'hov_doc_des_col', [
                'label'     => __( 'On Hover Doctor Designation Color', 'medisen' ),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
    ...
    $dynamic_class = is_front_page() ? 'doctor_part section_padding' : 'doctor_part section_padding single_doctor_part';
    ?>
            ...
            <div class="row">
                ...
                if( is_array( $doctors ) && count( $doctors ) > 0 ){
                    foreach ( $doctors as $doctor ) {
                        $doc_img   = !empty( $doctor['image']['id'] ) ? wp_get_attachment_image( $doctor['image']['id'], 'medisen_doctor_img_263x339', false, array( 'alt' => $doctor['doc_name'] ) ) : '';
                    ...
                    <div class="single_blog_item">
                        <div class="single_blog_img">
                            ...
                            <div class="social_icon">
                                ...
                                <a href="<?php echo esc_url( $skype );?>"> <i class="ti-skype"></i> </a>
                            </div>
                        </div>
